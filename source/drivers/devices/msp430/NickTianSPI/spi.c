...
int spi_enable(void)
{
	...
	//{
		...
		UCA0CTL = SWRST;					// Software reset USART0
		UCA0CTL |= CHAR + SYNC + MM;		// 8 bit characters, SPI Master mode
		UCA0MCTL = 0x00;					// Modulation control not used for SPI
		spi_set_speed(spi_cur_div);			// Set the clock speed
		UCA0TCTL |= CKPH + SSEL1 + STC;	// UCLK delayed half cycle, UCLK = SMCLK/U0BRx, 3 pin
